...
class Arguments:
    def __init__(self, env, agent):
        ...
        if self.if_off_policy:
            ...
            self.target_step = self.max_memo # repeatedly update network to keep critics loss small
            self.repeat_times = 2 **3 # collect target step then update
            self.if_per_or_gae = False # user PER: GAE for sparse rewards
        else:
            self.net_dim = 2 **9 # the network width
            self.max_memo = 2 ** 12 # replay buffer cap
            self.batch_size = self.net_dim * 2
