...
class AgentRecurrentPPO:
    ...
    def __init__(self, _net_dim=256, hidden_dim=126, _state_dim=8, _action_dim=2, _learning_rate=1e-4,
        ...
        self.network = ShareRecurrentPPO(_state_dim, _action_dim, hidden_dim, _net_dim)
    ...
    def select_actions(self, state: torch.Tensor,  hidden_state: torch.Tensor) -> torch.Tensor:
        ...
        action, value_tensor, hidden_state = self.network(state, hidden_state)
    ...
    def explore_one_env(self, env, target_step, reward_scale, gamma):
        ...
        state = self.states[0]
        hidden_state = self.hidden_states[0]
        ...
        while step < target_step:
            ...
            ten_actions, value_tensor, ten_hidden_state, ten_noises = self.select_actions(ten_states, ten_hidden_states)

            action = ten_actions[0].numpy()

            next_s, reward, done, _ = env.step(np.tanh(action))

            traj.append((ten_states, ten_hidden_states, reward, done, ten_actions, ten_noises))
            if done:
                state = env.reset()
                last_done = step
            else:
    ...
    def update_net(self, buffer, batch_size, repeat_times, soft_update_tau):
        ...
        with torch.no_grad():
            # buf_len = buffer[0].shape[0]
            buf_state, buf_hidden, buf_reward, buf_mask, buf_action, buf_noise = [ten.to(self.device) for ten in buffer]
            buf_len = buf_state.shape[0]
            '''get buf_r_sum, buf_logprob'''
            bs = 2 ** 10  # set a smaller 'BatchSize' when out of GPU memory.
            buf_value = [self.cri_target(buf_state[i:i + bs]) for i in range(0, buf_len, bs)]
        ...
        for update_i in range(1, update_times + 1):
            ...
            state = buf_state[indices]
            hidden_state = buf_state[indices]
            r_sum = buf_r_sum[indices]
            adv_v = buf_adv_v[indices]
            action = buf_action[indices]
            logprob = buf_logprob[indices]
